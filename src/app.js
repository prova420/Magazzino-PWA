...
const App = {
    ...
    initializeModules() {
        ...
        // 6. Airtable Sync - INIZIALIZZAZIONE SICURA CON TIMEOUT
    ...
    initializeAirtableSync() {
        if (typeof AirtableSync !== 'undefined') {
            ...
            console.warn('⚠️ AirtableSync non disponibile - tentativo di recupero...');
            
            // Tentativo di recupero più robusto
            const maxAttempts = 3;
            let attempts = 0;
            ...
            const tryInit = () => {
                ...
                if (typeof AirtableSync !== 'undefined') {
                    try {
                        ...
                        console.error('❌ Errore inizializzazione AirtableSync in ritardo:', error);
                    }
                } else if (attempts < maxAttempts) {
                    console.log(`🔄 Tentativo ${attempts}/${maxAttempts} per AirtableSync...`);
                    setTimeout(tryInit, 2000 * attempts);
                } else {
                    console.error('❌ Impossibile inizializzare AirtableSync dopo tutti i tentativi');
                }
            };
    ...
    finalValidation() {
        ...
        // Verifica configurazione Airtable
        if (typeof AirtableSync !== 'undefined' && AirtableSync.isConfigValid) {
            const isAirtableConfigured = AirtableSync.isConfigValid();
            console.log(isAirtableConfigured ? '✅ Airtable configurato' : 'ℹ️ Airtable non configurato');
        }
    ...
    showDebugInfo() {
        ...
        if (debugInfo) {
            const modules = ['Database', 'Notifications', 'ThemeManager', 'HistoryManager', 'SyncManager', 'AirtableSync', 'InventoryManager', 'UIManager'];
