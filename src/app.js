// src/app.js - VERSIONE COMPLETA E CORRETTA
const App = {
    init() {
    ...
    initializeModules() {
        ...
        // 6. Airtable Sync - con gestione robusta
    ...
    initializeAirtableSync() {
        // Inizializzazione ritardata e sicura per AirtableSync
        setTimeout(() => {
            if (typeof AirtableSync !== 'undefined') {
                if (AirtableSync.initialized) {
                    console.log('⚠️ AirtableSync già inizializzato');
                    return;
                }
                
                if (typeof AirtableSync.init === 'function') {
                    try {
                        AirtableSync.init();
                    ...
                    console.warn('⚠️ AirtableSync.init non è una funzione');
                }
            } else {
                console.warn('⚠️ AirtableSync non disponibile');
            }
        }, 500);
    },
    ...
    setupPWABanner() {
        // Banner informativo PWA opzionale
        ...
        if (!banner || !installBtn || !closeBtn) {
            return; // Banner non presente, skip
        }
        
        // Mostra il banner dopo 10 secondi (solo se non è già installata)
        setTimeout(() => {
            if (!this.checkIfAppIsInstalled() && localStorage.getItem('pwaBannerDismissed') !== 'true') {
